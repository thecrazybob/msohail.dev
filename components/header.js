    ...
    <header className="flex justify-between items-center mt-10 mb-32">
      ...
      <div className="flex space-x-4 items-center md:hidden">
        ...
        <button onClick={handleClick} aria-label="Mobile menu toggle">
          {active ? (
            <span className="relative text-white z-50 text-7xl rotate-45">
              +
            </span>
          ) : (
            <svg
      ...
      <div
        className={`${
          active
            ? "h-full w-full fixed z-40 left-0 top-0 bg-opacity-80 bg-black overflow-hidden transition duration-150"
            : "hidden"
        }`}
      >
        <div
          onClick={handleClick}
          className="flex flex-col h-full justify-center items-center text-gray-200 text-5xl space-y-6"
        >
